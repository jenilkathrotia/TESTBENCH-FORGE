...
A self-contained, dependency-free HTML page with a "Reveal saved result ▶" button that animates the
...
<div class="wrap">
  ...
  <button id="btn" onclick="reveal()">Reveal saved result: base → GRPO-trained ▶</button>
  <p class="foot">Animates the <b>saved</b> before→after from the real Modal A100 GRPO run (<code>grpo_result.json</code>) — a pre-computed result rendered for legibility, <b>not</b> live inference. The live, re-runnable proofs are <code>selftest.py</code> and <code>security_checks.py</code>.</p>
  <p class="foot">Trained on 7 modules, evaluated on <span class="hl">3 it never saw</span>. These bars are the <b>in-training</b> eval (n=5). The <b>reproducible</b> reload eval (n=16) is <span class="hl">0.23 → 0.79</span> held-out, <span class="hl">binary_search 0.31 → 0.93</span> — see <code>demo_suites.html</code> for the model's literal base-vs-trained suites. A transferable test-writing skill, not memorization.</p>
