...
Wraps `testbench.score_suite` as a reward function so the same scorer that evaluates
also trains the model via RFT/GRPO.

Eval Protocol signature, verified against the current local SDK:
    @reward_function
